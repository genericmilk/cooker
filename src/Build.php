...
class Build extends Command
{
     ...
     * @var string
     */
	protected $signature = 'build:res {--dev}';
	protected $bar;
	protected $dev;
     ...
     * @var string
     */
    protected $description = '👨‍🍳 Cooker 3 by Genericmilk. Run this to cook resources';
    ...
    public function handle(){
		...
		!config('cooker.silent') ? $this->info('👨‍🍳 Cooker 3'.PHP_EOL) : '';

		$this->dev = config('app.debug');
		if($this->option('dev')){
			$this->dev = $this->option('dev');
		}

		$this->dev ? $this->info('🔨 Running in development mode'.PHP_EOL) : '';
	...
	private function less_libr(){
		...
		try{
			...
			$libs = '';
			foreach($dir as $lib){
				$libs .= file_get_contents(resource_path('less/libraries/'.$lib)).PHP_EOL;
			}
			return $libs;
		}catch(\Exception $e){
			return null;
	...
	private function js_libr(){
		try{
			...
			$libs = '';
			foreach($dir as $lib){
				$libs .= file_get_contents(resource_path('js/libraries/'.$lib)).PHP_EOL;
			}
			return $libs;
		}catch(\Exception $e){
			return null;
			...
			foreach($frameworkList as $frameworkOnList){
				...
				if($frameworkOnList->type==$platform && $frameworkOnList->name==$f){
					...
					if (Cache::has($cache_name)) {						
						...
						$download = file_get_contents($this->dev ? $frameworkOnList->urlDev : $frameworkOnList->url).PHP_EOL;
