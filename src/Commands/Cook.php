...
use Genericmilk\Cooker\Preparsers\Frameworks;
...
class Cook extends Command
{
    ...
    public function handle(){
		...
		if(!config('cooker.silent') || $this->hasFailed){
			...
			if(count($this->failedToCook)>0){
				
				$cookedOk = count(config('cooker.ovens')) - count($this->failedToCook);

				if($cookedOk>0){
					// some cooked ok
					info('🟡 '.$cookedOk.' '.($cookedOk==1 ? 'Oven' : 'Ovens').' Cooked OK');
				}else{
					// all failed
					info('⚫️ '.$cookedOk.' '.($cookedOk==1 ? 'Oven' : 'Ovens').' Cooked OK');
				}

				info('🔴 '.count($this->failedToCook).' '.(count($this->failedToCook)==1 ? 'Oven' : 'Ovens').' Failed');
				
				// be helpful and show printout

				if(count($this->failedToCook)>0){
					echo PHP_EOL;
	
					alert('Some ovens failed to cook. Here\'s the details:');
					$this->table(['','Job', 'Status'],$this->failedToCook);
				}
				...
				$cookedOk = count(config('cooker.ovens')) - count($this->failedToCook);
				info('🟢 '.$cookedOk.' '.($cookedOk==1 ? 'Oven' : 'Ovens').' Cooked OK');
				note('⚫️ '.count($this->failedToCook).' '.(count($this->failedToCook)==1 ? 'Oven' : 'Ovens').' Failed');	
			}
	...
	private function runOven($job){
		try{
			...
			if($hasDiffs){
				...
				// Step 1: Get the frameworks
				$frameworks = $oven->format=='js' ? Frameworks::obtain($this->dev) : ''; // get the cook job's frameworks

				// Step 2: Get the packages
				$packages = $oven->format=='js' ?  Packages::obtain() : ''; // get the cook job's packages

				// Step 3: Get the preloads
				...
				// Step 5: Make a new output buffer for the appcode
				$appcode = ''; // start a new output buffer

				// Step 6: Add the toolbelt if JS
				if($oven->format=='js'){
				...
				$o = implode('',$o); // make the output buffer
