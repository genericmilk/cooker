...
class Build extends Command
{
    ...
    public function handle(){
		...
		!config('cooker.silent') ? $this->info('👨‍🍳 Cooker '.$this->version.' ('.$this->env.')') : '';

		// Run jobs
		$t = [];
		foreach(config('cooker.jobs') as $job){
			...
			$frameworks = Frameworks::obtain($job['frameworks'],$cooker->format,$this->dev);
			$libraries = $this->libraries($job['libraries']);
			$appcode = $cooker::cook($job); // custom resources			
			$appcode .= $cooker->format=='js' ? $job['namespace'].'.boot();' : '';
			$appcode = $this->compress($appcode,$cooker->format);
		...
		if(!config('cooker.silent')){
			...
			$this->notify('Cooker', 'Cooker finished cooking in xyzs',__DIR__.'/../../cooker.png');
		}
    }
